...
class KlanImages(KaitaiStruct):
    ...
    def _read(self):
        ...
        self.fat = KlanImages.TFat(self._io, self, self._root)
        self.data = KlanImages.TData(self._io, self, self._root)
    ...
    class TData(KaitaiStruct):
        ...
        def images(self):
            ...
            for i in range(8192):
                self._m_images[i] = KlanImages.TImage(self._parent.fat.offsets[i], self._io, self, self._root)
    ...
    class TImage(KaitaiStruct):
        ...
        def content(self):
            ...
            if self.param_offset != 0:
                ...
                self._m_content = KlanImages.TImageContent(self._io, self, self._root)
                self._io.seek(_pos)
    ...
    class TImageContent(KaitaiStruct):
        ...
        def _read(self):
            ...
            _on = self.mode
            if _on == 257:
                self.data = KlanImages.TImageDataIndexed(self.data_size, self._io, self, self._root)
            elif _on == 4:
                self.data = KlanImages.TImageDataLossy(self.data_size, self._io, self, self._root)
            elif _on == 1:
                self.data = KlanImages.TImageDataIndexed(self.data_size, self._io, self, self._root)
            elif _on == 5:
                self.data = KlanImages.TImageDataRgb565(self.data_size, self._io, self, self._root)
            elif _on == 258:
                self.data = KlanImages.TImageDataIndexed(self.data_size, self._io, self, self._root)
            elif _on == 256:
                self.data = KlanImages.TImageDataIndexed(self.data_size, self._io, self, self._root)
