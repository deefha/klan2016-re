    ...
    class TData(KaitaiStruct):
        ...
        def _read(self):
            ...
            for i in range(self.height):
                self.rows[i] = self._io.read_bytes(self.width)
    ...
    class TFont(KaitaiStruct):
        ...
        def data(self):
            ...
            if self.characters[i].width != 0:
                ...
                self._m_data = [None] * (256)
                for i in range(256):
                    self._m_data[i] = self._root.TData(self.characters[i].width, self.height, self._io, self, self._root)
